    ...
    try:
        ...
        if conn.is_connected():
            ...
            if len(pricing_data) > 0 and foundOthers:
                ...
                # # Find AF Supply's rank in the sorted pricing data
                # af_supply_rank = None
                # for idx, data in enumerate(pricing_data):
                #     if data.get('vendor_name', '').strip().lower() == 'af supply':
                #         af_supply_rank = idx + 1  # human-readable rank (1-indexed)
                #         break
                # # Update the rank in TempVendorPricing
                # if af_supply_rank:
                #     try:
                #         this.execute("""
                ...
                #         """, (af_supply_rank, current_vendor_pricing_id))
                #         conn.commit()
                #         logger.debug(f"Updated AF Supply rank to {af_supply_rank} for vendor_pricing_id {current_vendor_pricing_id}")
                #     except Exception as e:
                #         logger.debug(f"Error updating AF Supply rank: {e}")

                # rank_for_vndor = None
                # for idx, data in enumerate(pricing_data):
                #     vendor_product_id = data.get('vendor_product_id')
                #     rank_for_vndor = idx + 1  # human-readable rank (1-indexed)
                ...
                #             """, (rank_for_vndor, vendor_product_id))
                #             conn.commit()
                #             logger.debug(f"Updated rank to {rank_for_vndor} for vendor_product_id {vendor_product_id}")
                #         except Exception as e:
                #             # logger.debug(f"Error updating AF Supply rank: {e}")
                #             logger.debug(f"Error updating rank for vendor_product_id {vendor_product_id}: {e}")
                ...
                for key, val in rankedVendors.items():
                    ...
                    if column == total_columns:
                        ...
                        update_query += f" {key} = %s,"
                    column += 1
                update_query += f" WHERE vendor_product_id = {vendor_product_id};"
                ...
                conn.commit()
                if this.rowcount == 1:
                    logger.debug(f"Ranks are set null vendor_product_id ({vendor_product_id})")
                # updating ranking
                this.execute(update_query, update_values)
                conn.commit()
                if this.rowcount > 0:
                    logger.debug(f"Pricing data updated for vendor_pricing_id ({vendor_product_id}) ({vendor_id} X {product_id})")
...
def setAsPicked(vendor_product_id, atmpt=1):
    try:
        conn = mysql.connector.connect(host=HOST, database=DB, user=USER, password=PASS)
        if conn.is_connected():
            this = conn.cursor()
            this.execute("UPDATE ProductVendor SET update_ranking = %s WHERE vendor_product_id = %s;", ('2', vendor_product_id))
            conn.commit()
            logger.debug(f"Product (ID: {vendor_product_id}) is marked as picked")
    except mysql.connector.Error as e:
        if 'timeout' in str(e):
            if atmpt == 3:
                ...
                setAsPicked(vendor_product_id, atmpt=atmpt+1)
        else:
            ...
            if result:
                ...
                this.execute("""
                    ...
                    SET
                        ...
                        fifth_vendor_is_backorder = 'no', fifth_vendor_delivery_text_gmc = NULL, fifth_vendor_delivery_text_website = NULL, fifth_vendor_stock_text_website = NULL,
                        fifth_vendor_price_date = NULL, fifth_vendor_stock = NULL, percentage = NULL, second_percentage = NULL, assumed_map_price = NULL, is_rp_calculated = '0'
                    WHERE vendor_pricing_id = %s;""", (result[0],)
                )
                conn.commit()
                if this.rowcount == 1:
                    logger.debug(f"Ranks are set null vendor_pricing_id ({result[0]}) of history table.")
                # updating ranking
                update_query = update_query.replace('TempVendorPricing', pricing_history_table)
                update_query = update_query.split("WHERE")[0] + f" WHERE vendor_pricing_id = {result[0]};"
                this.execute(update_query, update_values)
                conn.commit()
                if this.rowcount == 1:
                    logger.debug(f"Pricing data updated for vendor_pricing_id ({result[0]}) in history table.")
            else:
                if len(pricing_data_for_history_list) > 0:
                    # inserting pricing data into history
                    this.execute("""
...
def products(product_id, vendor_id):
    try:
        conn = mysql.connector.connect(host=HOST, database=DB, user=USER, password=PASS)
        if conn.is_connected():
            this = conn.cursor()
...
def commence(productID, vendorID):
    start = time.perf_counter()
    logger.debug("""
    ------ Starting evaluating competitors ranking of the product ---------
    """)

    vendorsProducts = products(productID, vendorID)
